...
class WebGlobalCustom {
    constructor() {
        this.displaySliderArrows();
        this.displaySidebarIcons();
        this.gdprAlert();
        this.displayWhatsappButton();
        this.customLink();
        this.openAjaxModal();

        console.log('gogogo i2c');
    }
    ...
    displaySidebarIcons() {
        ...
        let elementOffset = sidebarIcons.getAttribute('data-offset');
        ...
        window.addEventListener('scroll', () => {
            if (window.scrollY > Number(elementOffset)) {
    ...
    initMasks() {
        ...
        }).mask('.kt_license_plate_ptbr_mask');
        ...
        Inputmask({
            ...
            'rightAlign': false,
            'placeholder': '0'
        }).mask('.kt_float_ptbr_mask');

        // DDD + Phone
        Inputmask({
            'mask': ['(99) 9999-9999', '(99) 99999-9999'],
            'keepStatic': true
        }).mask('.kt_phone_ptbr_mask');
    }
    ...
    clearForm(form) {
        for (var i = 0; i < form.elements.length; i++) {
            ...
            if (form.elements[i].type === "text" || form.elements[i].type === "email" || form.elements[i].type === "textarea" || form.elements[i].type === "password") {
...
if (!window.webCustom) {
    window.webCustom = new WebGlobalCustom();
}

export const webCustom = window.webCustom;

// const baseHref = webCustom.getBaseHref();
